...
if(user){
  ...
  stepNumber = 1;
  if(user.user_type === "department manager")
  stepNumber = 2;
  if(user.user_type === "building manager")
  stepNumber = 3;
  if(user.user_type === "police officer")
  stepNumber = 4;
}
...
const Requests = () => {
  ...
  const hiddenAreaSourceAndLayers = mapInstance => {
    ...
    mapInstance.on('mousemove', 'area-layer', (e) => {
      ...
      if (showTooltip) {
        ...
        mapInstance.setFilter('hidden-layer-border-hover', ['in', 'Shape_Area', '']);

    }
  });
  mapInstance.on('mouseleave', 'area-layer', function () {
      mapInstance.setFilter('hidden-layer-hover', ['in', 'ALIAS_NAME', '']);
      mapInstance.setFilter('hidden-layer-border-hover', ['in', 'ALIAS_NAME', '']);
  });
  ...
  const toggleView = () => {
    ...
    if(!viewIn3d){
        setZoom(19)
        setCenter([-89.355726877, 43.108446190]);
        setPitch(45);
        setBearing(-89);
    }else{
        setZoom(15)
  ...
  //  new code end here ...
    ...
    return (<>
      ...
      {(!showRequestDetailsModal && !prDisplay) && <div className="icons">
        <ul>
        { ( user.user_type === "building manager" || user.user_type === "department manager" ) && <li>
        ...
        <li>
          <button type="button" className="btn btn-outline-secondary" onClick={()=> alert("under development")}>
        ...
        <li>
          ...
          >
            <button type="button" className="btn btn-outline-danger pr-4 pl-4" onClick={() => {
          ...
          >
            <button type="button" className="btn btn-outline-info pr-4 pl-4" style={{borderRadius: 50}} onClick={toggleView}>
              { viewIn3d ? "3d" : "2d" }
            </button>
          </li>
        </ul>
      ...
      <div className="floor-list-container  text-center">
        <div className="p-2">Floor</div>
        <ul>
          <li className="floor-list-number" onClick={()=> alert("under development")}>
            <span>1</span>
          </li>
      ...
      {  ( user.user_type !== "student" ) && <div className="request-sidebar" id="sidebar" >
        { (requestList.length !== 0) ? <>
        { requestList.map((obj, index) => <RequestItem request={obj} key={index} setRequest={setRequestItemFunc}/>) }  </> : <h6 className="text-center text-danger">No pending requests!</h6>
        }
      </div> 
      }
      { ( showRequestDetailsModal === true ) && <RequestDetailsModal hideSRDM={hideSRDM} stepNumber={stepNumber} requestItemInfo={singleRequestItem} deleteRequestByRowNumber={deleteRequestByRowNumber} /> }
      {/* Modal */}
      {
        lockDownModal()
      }
